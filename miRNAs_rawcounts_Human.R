################################################################################
## This file does:                                                            ##
## 1. Extract miRNA reads from SPORTS output text files                       ##
...
#----[ Extract miRNA reads from SPORTS output text files ]------#
{
  files <- list.files(pattern="*.txt", full.names=TRUE, recursive=FALSE)
  for(x in files){
    temp <- read.table(x, header=TRUE) # load file
    temp <- temp[grep("hsa-miR|hsa-let-", temp$Annotation ), ]
    temp$ID <- NULL
    temp$Sequence <- NULL
    temp <- temp[temp$Length>=19,]
    temp$Length <- NULL
...
#----[ Merge miRNA reads from new cleaned text files ]------#
{
  ...
  merged_data <- purrr::reduce(imported_files, merge, by = "Annotation", all = TRUE)
  
  write.table( as.data.frame(merged_data), file="All_merge_miRNAs.txt", row.names = FALSE )
